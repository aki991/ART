...
export function PigeonsListView({ initialPigeons }: PigeonsListViewProps) {
  ...
  return (
    <>
      <div className="flex items-center justify-end mb-4 xl:mb-5 2xl:mb-6">
        <button
          type="button"
          onClick={openCreateModal}
          className="btn-shine-redesign inline-flex items-center gap-2 xl:gap-2.5 2xl:gap-3 px-4 xl:px-5 2xl:px-6 py-2.5 xl:py-2.5 2xl:py-3 rounded-md text-base xl:text-base 2xl:text-lg font-bold bg-accent text-text-on-accent hover:bg-accent-hover transition-colors whitespace-nowrap"
        >
          <Plus className="w-5 h-5 xl:w-5 xl:h-5 2xl:w-6 2xl:h-6" aria-hidden="true" />
          Dodaj goluba
        </button>
      </div>
      ...
      {pigeons.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-3 xl:gap-4 2xl:gap-6">
          {pigeons.map((pigeon) => (
            <PigeonCard
              key={pigeon.id}
              pigeon={pigeon}
