    ...
    <div className="card-redesign p-6">
      ...
      {recentRaces.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-1">Još nema trka.</p>
...
function RaceRow({ race, onClick }: { race: Race; onClick: () => void }) {
  ...
  return (
    ...
    >
      <td className="py-3 px-4 text-gray-900 font-medium text-sm">{race.name}</td>
      <td className="py-3 px-4 text-gray-700 text-sm">{formatDate(race.startedAt)}</td>
      <td className="py-3 px-4 text-gray-700 font-mono text-sm">
        {formatDuration(durationSeconds)}
      </td>
      <td className="py-3 px-4 text-gray-700 text-sm">{race.pigeons.length}</td>
