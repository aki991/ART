...
export function SessionStatsCard() {
  ...
  const avgAltitude = useMemo(() => {
    ...
    const sum = readings.reduce((acc, r) => acc + r.altitude, 0);
    return Math.round(sum / readings.length);
  }, [readings]);

  const avgDisplay = avgAltitude == null ? "—" : `${avgAltitude} m`;
  ...
  return (
    <div className="card-redesign p-3 xl:p-4 2xl:p-6 min-w-0">
      ...
      <div className="grid grid-cols-2 gap-2 xl:gap-2.5 2xl:gap-3 min-w-0">
        <div className="min-w-0">
          <p className="text-[10px] xl:text-[11px] 2xl:text-xs uppercase tracking-wide text-text-disabled mb-1 whitespace-nowrap">
            Trajanje sesije
          </p>
          <p className="text-lg xl:text-xl 2xl:text-2xl font-semibold text-accent whitespace-nowrap">
            {elapsed}
          </p>
        </div>
        <div className="min-w-0">
